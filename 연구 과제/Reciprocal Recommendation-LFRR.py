...
lev = 0           #각 사용자의 feature vector 크기를 비슷하게 맞추는 leveling 계수
lam = 1           #lambda: regularization 계수
...
def cost():
    ...
    for element in dataset_train.repeat().batch(2000).take(1):
        for row, col, rating in element.numpy():
            ...
            tmp = tf.square(rating - tf.squeeze(tf.matmul(fv1, fv2, transpose_b = True))) +                   lam * (tf.square(tf.norm(fv1)) + tf.square(tf.norm(fv2)))
            err = err + tmp
    return err
...
print(e/3000)
np.savetxt("test_error_lam.txt", test_error, fmt="%f", delimiter=' ')
...
print(e/10856)
np.savetxt("train_error_lam.txt", train_error, fmt="%f", delimiter=' ')
...
for element in dataset_test:
    ...
    col = element.numpy()[1]
    rating = element.numpy()[2]
    test_error.append([row, col, rating, np.dot(feature_from[row - 1], feature_to[col - 1])])
    e += np.square(rating -  np.dot(feature_from[row - 1], feature_to[col - 1]))
print(e/3000)
np.savetxt("test_error_lam.txt", test_error, fmt="%f", delimiter=' ')
...
for i in range(N):
    ...
    for j in heapq.nlargest(10, l):
        ...
        cnt += np.linalg.norm(feature_from[l.index(j)])
print(cnt / 10 * N)
